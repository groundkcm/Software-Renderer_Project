...
void CGameFramework::ProcessInput()
{
	...
	if (::GetKeyboardState(pKeyBuffer))
	{
		...
		if (pKeyBuffer['W'] & 0xF0) czKeyDelta = +0.05f;
		if (pKeyBuffer['S'] & 0xF0) czKeyDelta = -0.05f;
		if (pKeyBuffer['A'] & 0xF0) cxKeyDelta = -0.05f;
		if (pKeyBuffer['D'] & 0xF0) cxKeyDelta = +0.05f;
		if (pKeyBuffer[VK_PRIOR] & 0xF0) cyKeyDelta = +0.05f;
		if (pKeyBuffer[VK_NEXT] & 0xF0) cyKeyDelta = -0.05f;
